...
{
    [Fact]
    public async Task Given_NoSummaries_When_RankAsync_Then_DoNothing()
    {
        // Arrange
        ...
        realEstateAgentServiceMock.Setup(x => x.GetSummariesAsync(It.IsAny<string>(), cancellationToken))
        ...
        // Act
        await sut.RankAsync(cancellationToken);

        // Assert
        optionsMock.Verify();
        realEstateAgentServiceMock.Verify();
        forSaleRankingServiceMock.Verify(x => x.RankAsync(It.IsAny<List<ForSaleRankingModel>>(), cancellationToken), Times.Never);
    }

    [Fact]
    public async Task Given_OneSummary_When_RankAsync_Then_RankTheRealEstateAgent()
    {
        // Arrange
        ...
        realEstateAgentServiceMock.Setup(x => x.GetSummariesAsync(searchLocation, cancellationToken))
        ...
        optionsMock.Verify();
        realEstateAgentServiceMock.Verify();
        forSaleRankingServiceMock.Verify(x => x.RankAsync(It.Is<List<ForSaleRankingModel>>(x =>
            x[0].Name == realEstateAgentName &&
            x[0].ForSaleCount == forSaleCount), cancellationToken), Times.Once);
    }

    [Fact]
    public async Task Given_MultipleSummaries_When_RankAsync_Then_RankTheRealEstateAgents()
    {
        // Arrange
